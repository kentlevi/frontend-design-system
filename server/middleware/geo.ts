...
export default defineEventHandler(async (event) => {
	...
	type GeoCountryResponse = { countryCode?: string }
	...
	try {
		...
		const geo = await $fetch<GeoCountryResponse>(
		...
		const errorPayload = e as { message?: string }
		console.log('[geo] external lookup failed:', errorPayload?.message || e)
	}
})