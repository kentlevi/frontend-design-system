...
import { addUserAddress, deleteUserAddress, fetchUserAddresses, setAddressDefault, updateUserAddress } from "./api.service";
...
export async function setDefault(id: number) {
	...
	try {
		const response = await setAddressDefault(id)
