...
export function useAccountPoints() {
	...
	const summary = computed(() => ({
		...
		conversion_rate_label: account_point_summary.conversion_rate_label.startsWith('account.')
			? translate(account_point_summary.conversion_rate_label)
			: account_point_summary.conversion_rate_label,
	}));
	const challenges = computed(() =>
		challenge_items.value.map((challenge) => ({
			...challenge,
			name: translate(challenge.name),
			reward_copy: translate(challenge.reward_copy),
		}))
	);
	const ranks = computed(() =>
		account_point_ranks.map((rank) => ({
			...rank,
			rank_label: translate(rank.rank_label),
			name: translate(rank.name),
			spend_requirement: rank.spend_requirement.startsWith('account.') ? translate(rank.spend_requirement) : rank.spend_requirement,
			perks: rank.perks.map((perk) => translate(perk)),
			level_up_bonus_gifts: rank.level_up_bonus_gifts.map((gift) => translate(gift)),
		}))
	);
	...
	const logs = computed(() =>
		log_items.value.map((log) => {
			if (log.id === 'earned-order-12405077220') {
				return { ...log, title: `${translate(log.title)} (Order # 12405077220)` };
			}
			if (log.id === 'used-order-12405077220') {
				return { ...log, title: `${translate(log.title)} (Order # 12405077220)` };
			}
			if (log.id === 'quest-sticker-explorer') {
				return { ...log, title: `${translate(log.title)} (${translate('account.points.challenges.stickerExplorer.name')})` };
			}
			if (log.id === 'quest-first-order-victory') {
				return { ...log, title: `${translate(log.title)} (${translate('account.points.challenges.firstOrderVictory.name')})` };
			}
			if (log.id === 'rank-level-up') {
				return {
					...log,
					title: `${translate(log.title)} (${translate('account.points.tierName')})`,
					value: translate(log.value),
				};
			}
			return log;
		})
