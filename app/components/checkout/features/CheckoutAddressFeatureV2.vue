		...
		<div class="checkout-member-shipping-group">
			<div class="checkout-member-address-group">
				...
				<div ref="shipping_swap_wrapper_ref" class="checkout-member-shipping-swap-wrap">
					<SavedAddress v-if="is_member && !ship_to_another_address"/>
					<ManualAddress v-else/>
				</div>
			</div>
...
const { enter_duration_ms, leave_duration_ms } = useCheckoutFeatureTransition();

const { t, is_member, is_shipping_address_modal_open } = useCheckoutExperienceFeatureContext();
